...
CardProgress.prototype.startFile = function startFile(file, matches) {
  this._printMatches(matches);
  ctx.fg(0,0,0);
  this._printString(1, this.pageHeight -2, file+'...', this.pageWidth);
};
...
CardProgress.prototype.end = function end(matches) {
  ...
  this._printMatches(matches);
  ctx.fg(0,0,0);
  this._printString(1, this.pageHeight -2, 'Done. '+matches.totalFiles+' files. '+matches.total+' matches. '+ _.keys(matches.keys).length+' unique.', this.pageWidth);
  //cursor.horizontalAbsolute(0).eraseLine();
};
