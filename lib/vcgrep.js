...
var argv = require('yargs')
  ...
  .describe('progress', 'Progress feedback, one of: '+ _.keys(progressOptions))
  .boolean('recursive')
  .alias('recursive', 'r')
  ...
  .check(function(argv, options) {
    ...
    var availableProgressOptions = _.keys(progressOptions);
    if (!_.contains(availableProgressOptions, argv['progress'])) {
      throw new Error('Unknown --progress value: "'+argv['progress']+ '". Should be one of: '+availableProgressOptions);
    }
  })
  .argv;
...
function progressFunc() {
  var progressOption = argv['progress'];
  return progressOptions[progressOption] || noProgress;
}
