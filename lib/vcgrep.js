...
function searchFiles(callback) {
  ...
  api.searchFiles(re, files, options, callback);
}
...
function findFilesFilter(file) {
  ...
  var include = !argv.include || matchstick(argv.include, 'wildcard').match(basename);
  var exclude = argv.exclude && matchstick(argv.exclude, 'wildcard').match(basename);
  return shell.test('-f', file) && include && !exclude;
}
...
function processArguments() {
  return require('yargs')
    ...
    .string('exclude')
    .describe('exclude', 'Exclude files that match the wildcard')
    .boolean('global')
    .alias('global', 'g')
